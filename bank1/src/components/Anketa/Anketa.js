...
const Anketa = (props) => {
  ...
  const block_of_bankruptcyConclusion_Both = (Object.keys(conclusion).length > 0) && (
    (conclusion === 'Both') && (
      <>
      <p>На выбор Вам доступно два вида процедуры банкротства. Исходя из ваших ответов, вы можете выбрать как банкротство через суд, так и воспользоваться правом на упрощённую процедуру банкротства через МФЦ.</p>
...
<a className='results__doc' href='/documents'>Перейти к документам</a>
                ...
                <p className='anketa__text'>Для того, чтобы всё сделать грамотно, Вам необходимо правильно оформить все документы. Воспользуйтесь составленными нами шаблонами и полным пакетом документов, необходимыми для прохождения процедуры банкротства.</p>
